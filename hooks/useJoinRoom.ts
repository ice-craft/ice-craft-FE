...
const useJoinRoom = () => {
  ...
  const joinRoomHandler = useCallback(
    ...
    [loginErrorHandler, setRoomId, setIsEntry, userId, nickname]
  );
  ...
  }, [loginErrorHandler, setIsEntry, userId, nickname]);

  //NOTE - 빠른 입장 (랜덤 방 입장)
  const fastJoinRoomHandler = useCallback(() => {
    loginErrorHandler(() => {
      setIsEntry(true);
      socket.emit("fastJoinRoom", userId, nickname);
    });
