...
class Moderator {
  ...
  shufflePlayers() {
    for (let i = this.players.length - 1; i > 0; i--) {
      ...
      [this.players[i], this.players[j]] = [players[j], players[i]];
    }
  }

  //NOTE - 참감자들 중 한명 랜덤으로 뽑기
  getRandomParticipant() {
    shuffleParticipants(this.players);
  ...
  setRoles = () => {
    ...
    this.players.forEach((player) => {
      if (player.role === "마피아" || player.role === "시민") {
        ...
        if (player.isLived) {
          this.roles[player.role].push(player.index);
        }
      }

      this.roles[player.role] = player.index;
    });
  };
